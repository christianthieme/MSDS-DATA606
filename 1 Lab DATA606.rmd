...
_**Answer: The command you would use to extract the coutns of the girls who were baptized is as follows:**_
```{r}
arbuthnot$girls
...
_**Answer: There is definitely some significant variablity in the number of girls baptized over the years, especially between 1640 and 1660, however, besides those 20 years, it appears that the general trend is that more girls are baptized each year. Once you hit the 1700s, it appears that there is again some variability and perhaps a plateau as well.**_
...
_**The chart above makes it look like there is some considerable variability because of the scale of the y-axis, however, all of these values fall within the range of .505 and .535 which means the proporttion of boys did not swing too wildly from year to year. **_
...
_**Loading the present data frame:**_
```{r}
source("more/present.R")
```
...
min_year <- min(present$year) #1940
max_year <- max(present$year)
dimensions <- dim(present)
col_name <- names(present)

sprintf("The years in the data set range from %g to %g", min_year, max_year)
sprintf("The dimensions of the data frame are: %g x %g", dimensions[1], dimensions[2])
sprintf("The column names are: %s, %s, and %s", col_name[1],col_name[2],col_name[3] )
```
...
```{r}
arb <- sum(arbuthnot$boys) + sum(arbuthnot$girls)
pres <- sum(present$boys) + sum(present$girls)

count_higher <- pres - arb
percent_greater <- arb/pres

sprintf("Arbuthnot's count of subjects: %g and present data's count of subjects is %f", arb, pres)
```
_**The present day data countains many more subjects than Arbuthnot's data, which we would expect because population has continued to increase. As far as scale is concerned, these two data sets are not on the same scale. The present day data spans 62 years and Arbuthnot's spans 81 years. Additionally, Arbuthnot was looking at baptism records in London and the present day data is looking at actual birth records in the US. However, there are similarities. Both datasets look at a count of boys and girls for each year. As mentioned before the present day data set counts are much larger (millions) vs Arbuthnot's (tens of thousands)**__
...
proportion <- present$boys/(present$boys + present$girls)

ggplot(data = present) + 
  aes(x = year , y = proportion ) +
  geom_line() + 
  labs(title = "Proportion of Boys to Girls from 1940 to 2002")
```


_**Answer: Based on the above chart, it does not look like boys are being born in greater proportion to girls as Arbuthnot thought. However, it's hard to compare these two datasets because he was looking at baptismal records in London many years ago when record keeping practices were probably fairly loose. What I'm saying is, it is probably not a good idea to compare these two datasets, other than just to create a quick assumption. Additionally, his population is looking at London, where our data set is looking at the US. There very well could be some proportional differences between different geographies.**_
...
present <-  present %>% dplyr::mutate(total = boys + girls)

max_birth_year <- present %>% filter(total == max(total))

sprintf("The year with the highest total amount of births was %g. There were %f births.",
        max_birth_year$year, max_birth_year$total)
```
