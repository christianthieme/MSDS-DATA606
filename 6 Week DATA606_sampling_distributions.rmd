...
**The distribution has a median of 1,442 and a mean of 1,500. The range is 5,308. However, it appears that the majority of the distribution falls between 500-3,500. The distribution appears to be slightly right skewed.**
...
**The distribution has a median of 1,429 and a mean of 1,444. The range is 2,312. The distribution appears to be slightly right skewed. The distribution is actually very similar to the distribution above with a simialr mean, median, and similar IQR, however the full range of the data is more narrow as compared to the first distribution.**    
...
**The mean of samp2 (1,620) is is higher than samp1 (1,444). If we took two more samples, one of size 100 and one of size 1000, the one of size 1,000 would provide a more accurate estimation of the population mean (law of large numbers).**    
...
**There are 5,000 elements in sample_means50. The distribution of sample_means50 is very close to symmetrical. The mean is 1501 and the median is 1498, which indicates that we are very close to symmetry.I would expect the distribution to change to be even more symmetrical if we increased our sample size to 50,000.**    
...
for (i in 1:100) {
  n_samp <- sample(area, 50)
  sample_means_small[i] <- mean(n_samp)
}

sample_means_small
```
  
**There are 100 elements in this object because we looped 100 times. Each element represents the the mean of the 100 samples of 50 houses we took.**    
...
**When the sample size is larger, the center moves closer to the acutal population mean. The spread also becomes more narrow and more centered around the mean.**
...
```{r}
prices <- sample(price,50)
summary(price)
```
  
**It appears that the sample mean is $180,796, this is currently our best estimate of the population mean.**    
...
**Based on the above chart and the summary statistics, I would estimate that the population mean of price is somewhere within a couple thousand dollar of $180,575.**   

```{r}
mean(price)
```
**The actual mean of price is $180,796.10, which is very close to what we calculated with our samples. **
...
for(i in 1:5000){
  samp4 <- sample(price, 150)
  sample_means150[i] <- mean(samp4)
}
...
```
**The distribution looks to be fairly symmetrical with a mean of $180,842. The distribution has less spread than the distribution for a sample size of 50. We calculated the population mean at the end of question 2, it was $180,796.10.**
...
**Of the last two sampling distributions, the distribution from question 3 (with 5000 iterations of 150 samples) has smaller spread. You would definitely prefer a distribution with a smaller spread because the averages should converge closer and closer to the population mean with the more samples you collect.**
