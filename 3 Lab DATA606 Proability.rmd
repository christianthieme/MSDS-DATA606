...
**A streak of length 1 means that the player made one shot and then missed the next shot. There is one make and one miss in a streak of one. A streak of zero would be a miss with no makes.**
...
**Kobe's streak distribution is very right skewed. In additon, it appears that most of his streaks were either a streak of 0 (miss) or a streak of 1 (one make followed by a miss on the next shot). Typical streak was a streak of 1. It appears that his longest streak was a streak of 4 which would be 3 made shots followed by a miss. Looking at the distribution of the shots in the boxplot below, it is easy to see that the majority of streaks (75% of the values) fall between 0 and 1 while only 25% of his streaks are greater than 1. The spread of the data is quite narrow and it appears that the median value is 0. There are a few outliers, but as we can see from the bar graph above, these values look fairly infrequent, especially the streak of 4.**
...
## _**As a general note, as I had to re-run this workbook several times between working on the assignment over several days, some of the numbers will change based on running the random sample code, however, the numbers should not be materially different than what I had presented upon my first run of the code and description I've provided.**_
    
**In the similation above the first time I ran it I got 23 heads out of a 100, so just slightly more than 20%.**
...
**We need to change the prob argument to be prob = c(0.45,0.55)**

```{r}
#changing the probability of a made shot to 45% to match Kobe's shooting percentage.
outcomes <- c("H", "M")
...
**The distribution is again right skewed. However, this time we can definitely see that the spread of the data is wider than than Kobe's distribution. Creating a boxplot below, we can see the IQR ranges from 0-2 and it appears the median value is 1, where the median of Kobe's distribution appears to be 0. There also appear to be several outliers as well. Again, it appears that this simulated data has more spread and a few higher streak values (7 vs 4). It appears that the typical streak (median) is 1. The longest streak is 7, which is 6 made baskets before missing one. HOwever, all in all, excluding some of the outliers, it appears that the overall distribution is relatively similar.**
...
**If we were to run this simulation again, I would expect the distribution to be fairly/somewhat similar as we would obviously expect some random variation. I would expect this because our sample size is fairly large (133) - drawing on the law of large numbers, we will always expect the data to converge around the probabilities we have assigned, in this case 45% for makes and 55% for misses. There will of course be outliers and perhaps a slight change in spread just due to random chance, but the overall distribution will still remain fairly similar. I will rerun this simiulation below as a test:**    
    
```{r}
outcomes_1 <- c("H", "M")
sim_basket_1 <- sample(outcomes_1, size = 133, replace = TRUE, prob = c(0.45,0.55))
boxplot(calc_streak(sim_basket_1))
```
    
**Looking at the boxplot above, you can see that it is actually very close to Kobe's distribution and while there are some differences in spread compared to the boxplot shown earlier, the overall distribution remains fairly similar.**    
...
```{r}
streak_counts <- c(table(kobe_streak[1:74]), table(sim_streak))
#print(streak_counts)
barplot(streak_counts, main = "Kobe (on left) vs Independent Shooter (on right)")

```

**As I have discussed above throughout my responses, the distribution looks fairly similar to Kobe's distribution (see bar chart above). Based on this, it appears to provide evidence that each shot is independent and that the "hot hand" model does not fit Kobe's shooting patterns as his distribution is not materially different than the randomized model we built. While the data we've viewed here seems to dispute the "hot hand" assumption, I think having played basketball myself all through high school I would say that each shot is definitely not independent. When a shooter starts making/missing shots there is a definite pyscological effect that takes place, and the shooter either gains confidence or loses confidence, which then affects the players ability to perform/think/execute. Anyone who knows the game of basketball, knows its a game of momentum. To really look at the "hot hand" model, you can't look at shooting streaks as a only making a certain number of baskets and then a miss breaks the streak. For a shooter they could make two or three shots and then miss a shot and then make two shots and then miss one and then make two more and that would still be considered a hot hand. It should really be defined as a period of time where a player's shooting percentage is fairly high compared to their usual average. For example if a player makes 2 out of 3 shots, that a 66% average which is significantly higher than a typical player shoots, if a player goes on a streak and makes 2 out of 3 shots 3 or 4 times in a row, that would definitely be considered a hot hand. I think if you looked at the data in a a different way, maybe shooting percentage in 20 min increments (or count of makes every 10 min played), you would be able to provide evidence that each shot is not independent and that the "hot hand" exists.**
