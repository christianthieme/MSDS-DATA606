...
**The distribution is not normal. The mean value is 1,413, the range is 2,019 and the distribution appears to be right skewed. A typical house size appears to be around 1,413 sqr feet. Here I am designating the mean value as "typical"**

2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?

**I would not expect another student's distribution to be identical to mine as we are sampling 60 values from over 2,000. It could be similar, but it also could be significantly different. Because we are only sampling 60 values randomly and the range of values goes from 768, to 2,787 there is a possibility that more items were randomly selected from the lower end of the values or the higher end of the values that would make the distribution significantly different. **
...
**The observations need to be independent. Additionally, the sample size needs to be sufficiently large (n >=30)**
...
**95% confidence means that if we took many samples and built 95% confidence intervals around them, we would find the parameter (p) in 95% of them. **
...
**My confidence interval does capture the true average size of houses in Ames. Not working in a classroom.**
...
**I would expect around 95% of the intervals to capture the population mean because we are all building a confidence interval using 95% confidence. So ~95% of all intervals created in the classroom should capture the population parameter**
...
**98% of my intervals include the population mean (49/50). This proportion is not exactly equal to my confidence interval of 95%. There are several reasons why this isn't exact. For one, we are using simple random sampling and so are only trying to emulate a normal distribution, meaning our distributions may not actually be normal. If our distributions aren't fairly normal, our percentages will be skewed, however in this case it does look like we got fairly close.**
...
**I'm picking the critical value of 80%. The critical value is 1.281552. **
...
```{r}
lower_vector1 <- samp_mean - 1.281552 * samp_sd / sqrt(n) 
upper_vector1 <- samp_mean + 1.281552 * samp_sd / sqrt(n)
...
**It looks like 38/50 (76%) intervals contain the population mean. This percentage is fairly close to the 80% that I selected. We would expect some variation as well as knowing that this distribution is not perfectly normal.**
